...
from model_route_lint import __version__
from model_route_lint.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from model_route_lint.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='model-route-lint',
        description=(
                        'Lint model routing configs for missing fallbacks, budgets, and safety ti'
                        'ers.'
                    ),
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        help='model routing YAML, JSON, or notes',
    )
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"model-route-lint: error: {exc}", file=sys.stderr)
        return 1
